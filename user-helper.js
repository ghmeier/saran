...
module.exports.userList = (req, response) => {
    ...
    mongoService.getAllUsers(token, checked_in, (users) => {
        response.json({
            data: users
        });
        return;
...
module.exports.checkIn = (req, response) => {
    ...
    mongoService.getUser(id, (res) => {
        ...
        res.mlh_id = id;
        mongoService.putUser(id, res, (err) => {
            if (err) {
                response.json({
                    data: null,
                    error: err
...
module.exports.signup = function(req, response) {
    ...
    request({
        ...
        delete user.id;
        mongoService.getUser(user.mlh_id, function(res) {
            if (res) {
                user.checked_in = res.checked_in;
            }
            mongoService.putUser(user.mlh_id, user);
